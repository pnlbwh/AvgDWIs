...
//  /projects/schiz/software/Slicer3_forExternalLinking/Slicer3-build/bin/GenerateCLP --InputXML /projects/schiz/software/SlicerModules/AvgDWIs/src/AvgDWIs.xml --OutputCxx /projects/schiz/software/SlicerModules/AvgDWIs/src/AvgDWIsCLP.h
//
#include <stdio.h>
#include <stdlib.h>
#include <iostream>
...
"        <label>DWI's from which to create average DWI</label>      \n"
...
#define GENERATE_TCLAP \
    std::vector<std::string> volumeFileNamesTemp; \
    ...
    TCLAP::CmdLine commandLine (fullDescription, \
       ' ', \
      "0.1.0.$Revision: 8595 $(alpha)" ); \
 \
      itksys_ios::ostringstream msg; \
    msg.str("");msg << "File names.";    TCLAP::MultiArg<std::string > volumeFileNamesArg("", "volumeFileNames", msg.str(), 0, "std::vector<std::string>", commandLine); \
 \
    msg.str("");msg << "Output DWI volume.";    TCLAP::UnlabeledValueArg<std::string> outputVolumeArg("outputVolume", msg.str(), 1, outputVolume, "std::string", commandLine); \
 \
...
try \
 ...
 \
   ...
   for (ac = 0; ac < targs.size(); ++ac) \
    ...
    volumeFileNamesTemp = volumeFileNamesArg.getValue(); \
    ...
    returnParameterFile = returnParameterFileArg.getValue(); \
      { /* Assignment for volumeFileNames */ \
      for (unsigned int _i = 0; _i < volumeFileNamesTemp.size(); _i++) \
        { \
        std::vector<std::string> words; \
        std::vector<std::string> elements; \
        words.clear(); \
        splitFilenames(volumeFileNamesTemp[_i], words); \
        for (unsigned int _j= 0; _j < words.size(); _j++) \
          { \
            volumeFileNames.push_back((words[_j].c_str())); \
          } \
        } \
      } \
  } \
